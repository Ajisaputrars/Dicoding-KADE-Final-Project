...
class MatchDetailActivity: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        isNextMatch = getIntent().extras.getBoolean("isNextMatch")
        favoriteState()
        setUI()
    }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            ...
            R.id.menu_item_add_calendar_menu_detail_match -> {
                createEventOnCalendar(detailTeamObject, isNextMatch)
                return true
            }
            else -> {
    ...
    private fun createEventOnCalendar(event: Event, isNextMatch: Boolean) {
        if (isNextMatch == true) {
            ...
            intent.putExtra(CalendarContract.Events.DESCRIPTION,
                    "Reminder for match between ${event.homeTeam} and ${event.awayTeam}. Generated by Football Apps")

            startActivity(intent)
        } else {
            toast("Match had finished")
        }
    }
